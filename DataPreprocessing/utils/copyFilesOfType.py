...
def test(p1,p2):
    print("from", p1, "to", p2, " -> ", relpath(p1, p2))  # this is what I need. p1 = AbsDirPath; p2 = AbsfilePath; out = filepathRelToDir
...
def copyFiles(srcDir, destDir, extension, interactive=True):
    print("Source Dir: %s, Destination Dir: %s, Extension: %s" %(srcDir, destDir, extension))
    ...
    for root, dirs, files in os.walk(srcDir):
        for file_ in files:
            if file_.endswith(extension):
                ...
                # print(relSrcPath)
                destPath = os.path.join(destDir, relSrcPath)
    ...
    if ( interactive and (not query_yes_no("Are you sure you want to peform these operations?","yes"))):
        ...
        for i in tqdm(range(len(src))):
            ...
            if ( os.path.exists(dest[i]) ):
                # if (filecmp.cmp(src[i],dest[i]) == True):  #this reads all the files anyway, so doesn't make sense
                    # print("file exists, skipping")
                    # print("src: ", src[i],' Dst: ', dest[i])
                continue
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        dstDir = sys.argv[2]
        type = sys.argv[3]
        copyFiles(srcDir,dstDir,type, interactive=True)
